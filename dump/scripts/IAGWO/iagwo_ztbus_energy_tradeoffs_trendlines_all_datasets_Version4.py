...
PARAMETER_SPECS = [
    ...
    {
        "name": "eta_bus",
        "label": "Bus propulsion efficiency",
        "unit": "-",
        "default": 0.820,
        "lb": 0.630,
        "ub": 0.900,
...
def normalize_raw_dataset(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if accel_source is None:
        out["acceleration_m_s2"] = safe_gradient(
            finite_series(out["speed_m_per_s"]), finite_series(out["time_s"])
        )
    else:
    ...
    if mass_source is not None:
        ...
        if pax_source is not None:
            passengers = (
                pd.to_numeric(df[pax_source], errors="coerce").fillna(0).clip(0, MAX_PASSENGERS)
            )
...
def preprocess_dataset(use: pd.DataFrame) -> pd.DataFrame:
    ...
    use["power_measured_W"] = np.clip(
        finite_series(use["power_measured_W"]), -POWER_CLIP_W, POWER_CLIP_W
    )
    ...
    grade, alpha = compute_grade(
        use["distance_m"].to_numpy(dtype=float), use["elevation_m"].to_numpy(dtype=float)
    )
    use["grade"] = grade
...
def iagwo_optimize(
    datasets, n_wolves=24, n_iter=80, seed=42, progress_callback=None, progress_every=5
):
    ...
    wolves[0] = x0.copy()
    velocities = np.zeros_like(wolves)
    ...
    for it in range(n_iter):
        ...
        inertia = 0.90 - 0.50 * it / max(n_iter - 1, 1)
        ...
        for i in range(n_wolves):
            ...
            exploration = 0.05 * rng.normal(size=dim) * (ub - lb)
            velocities[i] = inertia * velocities[i] + 0.60 * (gwo_target - X) + exploration
            new_wolves[i] = np.clip(X + velocities[i], lb, ub)
        new_fitness = np.array(
            [objective_from_params(w, datasets) for w in new_wolves], dtype=float
        )
        improved = new_fitness < fitness
        wolves[improved] = new_wolves[improved]
        fitness[improved] = new_fitness[improved]
        if progress_callback is not None and (
            (it + 1) % max(progress_every, 1) == 0 or it == n_iter - 1
        ):
    ...
    return (
        wolves[0],
        float(fitness[0]),
        pd.DataFrame(history, columns=["iteration", "best_objective"]),
    )
    ...
    for name, df in datasets:
        ...
        rows.append(
            [
                ...
                name,
                len(pred),
                float(np.mean(pred["speed_m_per_s"]) * 3.6),
                float(np.mean(np.abs(pred["acceleration_m_s2"]))),
                float(np.mean(np.abs(pred["grade"])) * 100.0),
                float(np.mean(pred["mass_kg"]) / 1000.0),
                float(np.mean(pred["P_aux_W"]) / 1000.0),
                float(np.mean(pred["P_prop_W"]) / 1000.0),
                rmse,
                mae,
                r2,
                e_meas,
...
def plot_global_energy_bars(metrics_df):
    ...
    axs[0].hist(
        df["energy_measured_kWh"], bins=bins, alpha=0.65, color="tab:blue", label="Measured energy"
    )
    axs[0].hist(
        df["energy_model_kWh"], bins=bins, alpha=0.65, color="tab:orange", label="Model energy"
    )
    ...
    if len(monthly) > 0:
        ...
        axs[1].plot(
            x,
            polynomial_trend(x, summary["measured_kWh"], degree=2),
            color="tab:blue",
            lw=2.8,
            ls="--",
        ...
        axs[1].plot(
            x,
            polynomial_trend(x, summary["model_kWh"], degree=2),
            color="tab:orange",
            lw=2.8,
            ls="--",
            label="Model trend",
...
def plot_force_components_all_samples(all_df):
    ...
    ax.bar(
        ["Rolling", "Aerodynamic", "Inertia", "Grade"],
        vals,
...
def plot_energy_param_scatter(metrics_df, params_df):
    ...
    for ax, (xcol, xlbl, color, ttl) in zip(axs, views):
        sub = (
            df[[xcol, "energy_measured_kWh", "energy_model_kWh"]]
            .dropna()
            .sort_values(xcol)
        ...
        ax.plot(
            x,
            polynomial_trend(x, y_meas, degree=2),
            color=color,
            lw=2.7,
            ls="--",
            label="Measured trend",
        ...
        ax.plot(
            x,
            polynomial_trend(x, y_model, degree=2),
            color="tab:red",
            lw=2.7,
            ls="--",
            label="Model trend",
    ...
    for i, bus_id in enumerate(pivot.columns):
        ...
        ax.plot(
            x,
            polynomial_trend(x, vals, degree=2),
            color=f"C{i}",
            lw=2.7,
            ls="--",
            label=f"Bus {bus_id} trend",
        )
    ax.set_xticks(x)
    ax.set_xticklabels(pivot.index, rotation=25, ha="right")
    ax.set_title("IAGWO parameter profile trendlines")
...
def plot_performance_trendlines_monthly(metrics_df, bus_order):
    ...
    if len(df) == 0:
        return
    monthly = (
        df.groupby(["bus_id", "month"], dropna=False)
        .agg(
            rmse_W=("rmse_W", "mean"),
            mae_W=("mae_W", "mean"),
            r2=("r2", "mean"),
            abs_energy_error_pct=(
                "energy_error_pct",
                lambda x: np.nanmean(np.abs(pd.to_numeric(x, errors="coerce"))),
            ),
        )
        .reset_index()
    )
...
def plot_performance_distribution_histograms(metrics_df, bus_order):
    ...
    for ax, col, ttl in zip(axs, metric_cols, titles):
        for i, bus_id in enumerate(bus_order):
            vals = pd.to_numeric(df.loc[df["bus_id"] == bus_id, col], errors="coerce").to_numpy(
                dtype=float
            )
...
def build_statistical_benchmark(all_metrics_df, bus_order):
        ...
        for m in metric_cols:
            ...
            per_bus_rows.append(
                [
                    ...
                    float(np.percentile(vals, 10)) if len(vals) else np.nan,
                    float(np.percentile(vals, 90)) if len(vals) else np.nan,
                    ci_lo,
                    ci_hi,
                ]
            )
    ...
    if len(bus_order) >= 2:
        ...
        for m in metric_cols:
            x = pd.to_numeric(
                all_metrics_df.loc[all_metrics_df["bus_id"] == b1, m], errors="coerce"
            ).to_numpy(dtype=float)
            y = pd.to_numeric(
                all_metrics_df.loc[all_metrics_df["bus_id"] == b2, m], errors="coerce"
            ).to_numpy(dtype=float)
            x = x[np.isfinite(x)]
            y = y[np.isfinite(y)]
    ...
    comp_df = pd.DataFrame(
        comp_rows, columns=["metric", "reference_bus", "comparison_bus", "mean_diff", "cohen_d"]
    )
...
def plot_bus_performance_comparison(all_metrics_df, bus_order):
    ...
    for i, (m, ttl) in enumerate(zip(metric_cols, titles)):
        ...
        for bus_id in bus_order:
            vals = pd.to_numeric(
                all_metrics_df.loc[all_metrics_df["bus_id"] == bus_id, m], errors="coerce"
            ).to_numpy(dtype=float)
            vals = vals[np.isfinite(vals)]
            if len(vals):
                samples.append(vals)
    ...
    fig.suptitle("IAGWO performance comparison across buses", fontsize=13)
    fig.tight_layout()
...
def plot_bus_parameter_comparison(param_df):
    ...
    ax.bar(
        x - width / 2, pivot[buses[0]], width, label=f"Bus {buses[0]}", color="tab:blue", alpha=0.85
    )
    ax.bar(
        x + width / 2,
        pivot[buses[1]],
    ...
    ax.set_title("Estimated parameter comparison (IAGWO)")
    ax.legend()
    fig.tight_layout()
...
def plot_iagwo_vs_actual_parameter_histogram(param_df, bus_order):
    ...
    ax.set_title("IAGWO parameters vs actual bus parameters")
    ax.grid(alpha=0.25, axis="y")
    ax.legend(ncol=2, fontsize=9)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "iagwo_vs_actual_parameters_histogram.png", dpi=220)
    plt.close(fig)
...
def plot_energy_histogram_all_datasets(metrics_df):
    ...
    if len(df) == 0:
        return
    summary = (
        df.groupby("bus_id", dropna=False)
        .agg(
    ...
    ax.plot(
        x,
        summary["measured_median_kWh"],
        color="tab:blue",
        lw=2.0,
        ls="--",
        marker="o",
        label="Measured median trend",
    )
    ax.plot(
        x,
        summary["model_median_kWh"],
        color="tab:orange",
        lw=2.0,
        ls="--",
        marker="o",
        label="Model median trend",
    )
    ax.set_ylabel("Energy (kWh)")
...
def plot_bus_physical_averages(all_df, bus_order):
    ...
    df = df.replace([np.inf, -np.inf], np.nan).dropna(
        subset=["bus_id", "file", "speed_m_per_s", "temperature_C"]
    )
    if len(df) == 0:
        return
    ...
    agg = (
        ...
        .agg(
            avg_speed_kmh=(
                "speed_m_per_s",
                lambda x: float(np.nanmean(pd.to_numeric(x, errors="coerce")) * 3.6),
            ),
            avg_temperature_C=(
                "temperature_C",
                lambda x: float(np.nanmean(pd.to_numeric(x, errors="coerce"))),
            ),
            avg_abs_roll_kN=(
                "F_roll_N",
                lambda x: float(np.nanmean(np.abs(pd.to_numeric(x, errors="coerce"))) / 1000.0),
            ),
            avg_abs_aero_kN=(
                "F_aero_N",
                lambda x: float(np.nanmean(np.abs(pd.to_numeric(x, errors="coerce"))) / 1000.0),
    ...
    for bus_id in bus_order:
        ...
        axs[0].plot(
            x,
            polynomial_trend(x, sub["avg_speed_kmh"], degree=2),
            color="tab:orange",
            lw=2.8,
            ls="--",
            label="Speed trend",
        ...
        axs[0].plot(
            x,
            polynomial_trend(x, sub["avg_temperature_C"], degree=2),
            color="tab:red",
            lw=2.8,
            ls="--",
        ...
        for col, color, label in [
            ...
            axs[1].plot(
                x,
                polynomial_trend(x, sub[col], degree=2),
                color=color,
                lw=2.5,
                ls="--",
                label=f"{label} trend",
        ...
        stacked = np.vstack(
            [pd.to_numeric(sub[c], errors="coerce").to_numpy(dtype=float) for c in force_cols]
        )
        y_force_mean = np.nanmean(stacked, axis=0)
        axs[2].plot(x, y_force_mean, color="tab:purple", alpha=0.45, lw=1.2)
        axs[2].plot(
            x,
            polynomial_trend(x, y_force_mean, degree=2),
            color="tab:purple",
            lw=2.8,
            ls="--",
            label="Mean force trend",
        )
        axs[2].plot(x, sub["avg_abs_total_kN"], color="black", alpha=0.45, lw=1.2)
        axs[2].plot(
            x,
            polynomial_trend(x, sub["avg_abs_total_kN"], degree=2),
            color="black",
            lw=2.8,
            ls="--",
            label="Total force trend",
        )
...
def plot_energy_scatter_by_bus(all_df):
    ...
    df = df.replace([np.inf, -np.inf], np.nan).dropna(
        subset=["power_measured_W", "P_model_W", "bus_id"]
    )
    if len(df) == 0:
        return
    ...
    for bus_id, sub in df.groupby("bus_id"):
        ...
        ax.scatter(
            sub["power_measured_W"] / 1000.0,
            sub["P_model_W"] / 1000.0,
            alpha=0.25,
            s=8,
            label=f"Bus {bus_id}",
        )
    lim = np.nanmax(
        np.abs(
            np.concatenate(
                [
                    (df["power_measured_W"] / 1000.0).to_numpy(),
                    (df["P_model_W"] / 1000.0).to_numpy(),
                ]
            )
        )
    )
...
def plot_dataset_diagnostics(sample_name, pred_df):
    ...
    for c in numeric_cols:
        if c in df.columns:
            df[c] = pd.to_numeric(df[c], errors="coerce")
    df = df.replace([np.inf, -np.inf], np.nan).dropna(
        subset=["time_s", "power_measured_W", "P_model_W"]
    )
    if len(df) == 0:
        return
    ...
    measured_kw = plot_filter_series(
        plot_df["power_measured_W"] / 1000.0, hard_clip=PLOT_POWER_LIMIT_KW
    ).to_numpy()
    model_kw = plot_filter_series(
        plot_df["P_model_W"] / 1000.0, hard_clip=PLOT_POWER_LIMIT_KW
    ).to_numpy()
    residual_kw = plot_filter_series(
        plot_df["power_error_W"] / 1000.0, hard_clip=PLOT_RESIDUAL_KW_LIMIT
    ).to_numpy()

    prop_kw = plot_filter_series(
        plot_df["P_prop_W"] / 1000.0, hard_clip=PLOT_POWER_LIMIT_KW
    ).to_numpy()
    ...
    grade_pct = plot_filter_series(
        plot_df["grade"] * 100.0, hard_clip=PLOT_GRADE_LIMIT_PCT
    ).to_numpy()
...
def main():
    ...
    for i, bus_id in enumerate(BUS_IDS):
        ...
        best_theta, best_obj, history_df = iagwo_optimize(
            datasets,
            n_wolves=24,
            n_iter=80,
            seed=42 + i,
        ...
        metrics_df, pred_all_df, best_params = evaluate_all_samples(
            best_theta, datasets, bus_id=bus_id
        )
        ...
        bus_summary = pd.DataFrame(
            {
                ...
                "mean_abs_energy_error_kWh": [
                    pd.to_numeric(metrics_df["energy_error_kWh"], errors="coerce").abs().mean()
                ],
                "mean_abs_energy_error_pct": [
                    pd.to_numeric(metrics_df["energy_error_pct"], errors="coerce").abs().mean()
                ],
            }
        )
        bus_summary.to_csv(OUT_DIR / f"iagwo_bus_{bus_id}_summary.csv", index=False)
        pd.DataFrame(
            {"parameter": list(best_params.keys()), "value": list(best_params.values())}
        ).to_csv(OUT_DIR / f"iagwo_bus_{bus_id}_best_parameters.csv", index=False)
        metrics_df.to_csv(OUT_DIR / f"iagwo_bus_{bus_id}_sample_metrics.csv", index=False)
        log_progress(f"Bus {bus_id}: outputs saved")

    if not all_bus_metrics:
        raise FileNotFoundError(
            f"No usable parquet files found in {DATASET_DIR / 'bus=183'} and {DATASET_DIR / 'bus=208'}"
        )

    log_progress("Building global summaries and plots")
    metrics_all_df = pd.concat(all_bus_metrics, ignore_index=True).replace(
    ...
    predictions_all_df = (
        pd.concat(all_bus_predictions, ignore_index=True)
        if len(all_bus_predictions) > 0
        else pd.DataFrame()
    )
    stats_per_bus_df, stats_comp_df = build_statistical_benchmark(metrics_all_df, bus_order)

    params_pivot = params_all_df.pivot(
        index=["parameter", "description", "unit", "default", "lower_bound", "upper_bound"],
        columns="bus_id",
        values="best_estimate",
    ).reset_index()
    if (
        len(bus_order) >= 2
        and bus_order[0] in params_pivot.columns
        and bus_order[1] in params_pivot.columns
    ):
        params_pivot["delta_bus2_minus_bus1"] = (
            params_pivot[bus_order[1]] - params_pivot[bus_order[0]]
        )

    metrics_all_df.to_csv(OUT_DIR / "iagwo_two_bus_sample_metrics.csv", index=False)
    params_all_df.to_csv(OUT_DIR / "iagwo_parameter_estimates_long.csv", index=False)
    params_pivot.to_csv(OUT_DIR / "iagwo_parameter_estimates_comparison_table.csv", index=False)
    convergence_all_df.to_csv(OUT_DIR / "iagwo_two_bus_convergence.csv", index=False)
    stats_per_bus_df.to_csv(OUT_DIR / "iagwo_statistical_benchmark_per_bus.csv", index=False)
    stats_comp_df.to_csv(OUT_DIR / "iagwo_statistical_benchmark_comparison.csv", index=False)

    overall_summary = (
        metrics_all_df.groupby("bus_id", dropna=False)
        .agg(
            n_files=("file", "count"),
    ...
    overall_summary.to_csv(OUT_DIR / "iagwo_two_bus_summary.csv", index=False)
    ...
    plot_iagwo_vs_actual_parameter_histogram(params_all_df, bus_order)
