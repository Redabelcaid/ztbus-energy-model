...
def iagwo_optimize(datasets, n_wolves=24, n_iter=80, seed=42, progress_callback=None, progress_every=5):
...
def plot_iagwo_vs_actual_parameter_histogram(param_df, bus_order):
    ...
    for i, bus_id in enumerate(buses[:2]):
        ...
        ax.bar(x + offset, est_vals, width=width, alpha=0.85, label=f'Bus {bus_id} IAGWO', color=f'C{i}')
        ax.bar(
            x + offset + width,
            actual_vals,
            width=width,
...
def main():
    ...
    for i, bus_id in enumerate(BUS_IDS):
        ...
        pd.DataFrame({'parameter': list(best_params.keys()), 'value': list(best_params.values())}).to_csv(OUT_DIR / f'iagwo_bus_{bus_id}_best_parameters.csv', index=False)
