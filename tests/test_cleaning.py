...
def _synthetic_mission(
    n: int = 3601, *, bad_speed: bool = False, bad_power: bool = False
) -> pl.DataFrame:
    ...
    return pl.DataFrame(
        {
            "time_iso": [
                f"2020-01-01T{(s % 86400) // 3600:02d}:{(s % 3600) // 60:02d}:{s % 60:02d}+00:00"
...
def test_clean_mission_flags_speed_outliers() -> None:
    ...
    _cleaned, qc = clean_mission(df, mission_id="test_clean_02", bus=183, cfg=CleaningConfig())
...
def test_clean_mission_flags_power_outliers() -> None:
    ...
    _cleaned, qc = clean_mission(df, mission_id="test_clean_03", bus=183, cfg=CleaningConfig())
...
def test_clean_mission_rejects_non_monotonic_time() -> None:
    ...
    _cleaned, qc = clean_mission(df, mission_id="test_clean_04", bus=183, cfg=CleaningConfig())
    assert qc.rejected
...
def test_clean_mission_dedupes_timestamps() -> None:
    ...
    cleaned, _qc = clean_mission(df, mission_id="test_clean_05", bus=183, cfg=CleaningConfig())
